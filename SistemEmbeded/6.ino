...
// ==== STRUCT UNTUK FRAME ANIMASI MATA ====
struct EyeFrame {
  int pupilOffsetX;
  int pupilOffsetY;
  bool blink;
...
EyeFrame frames[] = {
  {0, 0, false},   // Normal
  {2, 0, false},   // Gerak kanan sedikit
  {4, 0, false},   // Gerak kanan lebih
  {2, 0, false},   // Kembali
  {0, 0, false},   // Normal
  {0, -1, false},  // Gerak turun sedikit
  {0, 0, false},   // Normal
  {0, 0, true},    // Blink
  {0, 0, false}    // Kembali normal
};
const int numFrames = sizeof(frames) / sizeof(frames[0]);
int eyeFrameIndex = 0;
const unsigned long eyeAnimInterval = 200; // ms untuk animasi halus
...
// ==== ANIMASI MATA DACSAI MOCHI (DIMODIFIKASI DENGAN FRAME BERURUTAN) ====
void drawEyeAnimation() {
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);

  // Posisi mata: disesuaikan untuk OLED 128x64
  int eyeY = 22;
  int radius = 12;
  int eyeX1 = 40;  // Mata kiri
  int eyeX2 = 88;  // Mata kanan (disesuaikan agar tidak overlap dengan tepi)

  EyeFrame &f = frames[eyeFrameIndex];

  // Gambar mata kiri
  display.fillCircle(eyeX1, eyeY, radius, SSD1306_WHITE);
  display.fillCircle(eyeX1, eyeY, radius - 2, SSD1306_BLACK); // Iris hitam
  if (!f.blink) {
    // Pupil putih dengan offset dari frame
    display.fillCircle(eyeX1 + f.pupilOffsetX, eyeY + f.pupilOffsetY, 6, SSD1306_WHITE);
    // Titik hitam kecil di pupil untuk efek mata
    display.fillCircle(eyeX1 + f.pupilOffsetX + 2, eyeY + f.pupilOffsetY - 1, 2, SSD1306_BLACK);
  } else {
    // Efek blink: garis tipis untuk mata tertutup
    display.drawLine(eyeX1 - 6, eyeY, eyeX1 + 6, eyeY, SSD1306_BLACK);
  }

  // Gambar mata kanan (simetris)
  display.fillCircle(eyeX2, eyeY, radius, SSD1306_WHITE);
  display.fillCircle(eyeX2, eyeY, radius - 2, SSD1306_BLACK);
  if (!f.blink) {
    display.fillCircle(eyeX2 + f.pupilOffsetX, eyeY + f.pupilOffsetY, 6, SSD1306_WHITE);
    display.fillCircle(eyeX2 + f.pupilOffsetX + 2, eyeY + f.pupilOffsetY - 1, 2, SSD1306_BLACK);
  } else {
    display.drawLine(eyeX2 - 6, eyeY, eyeX2 + 6, eyeY, SSD1306_BLACK);
  }

  // Update frame index hanya jika waktu sudah tiba (untuk timing halus)
  unsigned long now = millis();
  if (now - lastEyeAnim >= eyeAnimInterval) {
    lastEyeAnim = now;
    eyeFrameIndex = (eyeFrameIndex + 1) % numFrames;
  }

  // Tambahan: Nama kota & status WiFi di bawah (tidak overlap)
  ...
  // Mulut kecil lucu (dari kode asli, opsional)
  int mouthY = 50;
  if (eyeFrameIndex % 4 < 2) {
    // Garis lurus
    display.drawLine(64 - 4, mouthY, 64 + 4, mouthY, SSD1306_WHITE);
  } else {
    // Senyum kecil
    display.drawPixel(64 - 5, mouthY, SSD1306_WHITE);
    display.drawPixel(64 - 3, mouthY + 1, SSD1306_WHITE);
    display.drawPixel(64 + 3, mouthY + 1, SSD1306_WHITE);
    display.drawPixel(64 + 5, mouthY, SSD1306_WHITE);
  }
...
void loop() {
  ...
  if (now - lastDisplayUpdate >= displayInterval) {
    ...
    switch (currentSlide) {
      case 0: drawEyeAnimation(); break;  // Ganti dari drawMochiAnimation ke drawEyeAnimation
